    ...
    for (int i = 0; i < num_atoms; i++) {
        ...
        for (int j = i + 1; j <num_atoms; j++) {
            ...
            if(dist_ij < cutoff) {
                ...
                //std::cout << interaction_energy << std::endl;
                //std::cout << dist_ij << std::endl;
            }
        }
    }
...
Coordinates run_simulation(Coordinates coordinates, const double box_length, const double cutoff,
    ...
    for(int i = 0; i < num_steps; i++) {
        ...
        //std::cout << current_energy << ' ' << proposed_energy << std::endl;
...
int main(void) {
    ...
    std::cout << accept_or_reject(1, 100) << std::endl;

    std::cout << "calculate_distance tests: " << std::endl;
    std::cout << calculate_distance({0, 0, 0}, {0, 0, 1}, 10) << std::endl;
    std::cout << calculate_distance({0, 0, 0}, {0, 0, 8}, 10) << std::endl;
    ...
    Coordinates coords2;
    coords2.push_back({0, 0, 0});
    coords2.push_back({0, 0, pow(2, 1.0 / 6.0)});
    coords2.push_back({0, 0, 2.0 * pow(2, 1.0 / 6.0)});

    std::cout << coords2[1][2] << std::endl;
